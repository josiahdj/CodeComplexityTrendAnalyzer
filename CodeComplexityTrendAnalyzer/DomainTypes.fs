...
// Git
type CommitInfo = { Hash: string; Date: DateTime option; Author: string }
...
module ComplexityStats =
    let private zero = { Count = 0; Total = 0; Min = 0; Max = 0; Mean = 0.0; StdDev = 0.0 }
...
type InitialMemberInfo = { Name: string; Type: MemberType; Parameters: string option; }
with 
    member private _.ParametersToCompare (pl : ParameterListSyntax) = 
        pl.Parameters.ToList() |> Seq.map (fun p -> p.ToString()) |> Seq.sort |> String.concat ","

    member private this.Params
        with get () = this.Parameters |> Option.defaultValue String.Empty

    member this.FindPropertyDeclaration(ast: SyntaxTree) : Result<PropertyDeclarationSyntax, FindDeclarationError> = 
        let ms = ast.GetRoot().DescendantNodes().OfType<PropertyDeclarationSyntax>().Where(fun x -> x.Identifier.Text = this.Name)
        ms
        |> Seq.tryExactlyOne
    ...
    member this.FindMethodDeclaration(ast: SyntaxTree) : Result<MethodDeclarationSyntax, FindDeclarationError> = 
        let ms = ast.GetRoot().DescendantNodes().OfType<MethodDeclarationSyntax>().Where(fun x -> x.Identifier.Text = this.Name)
        if ms.Any() then
            ms 
        ...
        else Error (MethodNotFoundByName ms)

    member this.FindConstructorDeclaration(ast: SyntaxTree) : Result<ConstructorDeclarationSyntax, FindDeclarationError> = 
        let ms = ast.GetRoot().DescendantNodes().OfType<ConstructorDeclarationSyntax>().Where(fun x -> x.Identifier.Text = this.Name)
        if ms.Any() then
            ms 
            |> Seq.tryFind (fun x -> (this.ParametersToCompare x.ParameterList) = this.Params)
            |> Option.orElseWith (fun () -> ms |> Seq.tryExactlyOne) // assume that the signature was changed and that there's only one instance (if there are overrides, all bets are off)
            |> Result.ofOption (ConstructorNotFoundWithParams ms)
        else Error (ConstructorNotFoundByName ms)
        ...
        try
            ...
            | Property -> 
                result {
                    ...
                    return p  |> ((fun s -> s.ToString()) >> Strings.splitLines)
                }
            | Method ->
                result {
                    let! m = 
                        ...
                        |> Result.mapError (fun err ->
                            let ms = FindDeclarationError.toMethodDeclarations err
                            let meths = ms |> Seq.map (fun m -> m.ParameterList.Parameters.ToString()) |> List.ofSeq
                            MemberNotFound(this.Name, this.Type, this.Params, meths))
                    return m |> ((fun s -> s.ToString()) >> Strings.splitLines)
            ...
            | Constructor ->
                result {
                    let! c = 
                        ...
                        |> Result.mapError (fun err ->
                            let ms = FindDeclarationError.toConstructorDeclarations err
                            let meths = ms |> Seq.map (fun m -> m.ParameterList.Parameters.ToString()) |> List.ofSeq
                            MemberNotFound(this.Name, this.Type, this.Params, meths))
                    return c |> ((fun s -> s.ToString()) >> Strings.splitLines)
...
[<StructuralComparison>]
[<StructuralEquality>]
type CompleteMemberInfo = { Name: string; Type: MemberType; Parameters: string option; LineCount: int option; Complexity: Result<ComplexityStats, ComplexityError> }

type MemberRevision = { Commit: CommitInfo; Member: CompleteMemberInfo; LinesAdded: int; LinesRemoved: int }
    with 
        ...
        member this.Row = 
            ...
            let complexity = this.Member.Complexity |> Result.bimap id (fun e -> ComplexityStats.create List.empty)
